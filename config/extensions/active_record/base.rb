module CustomizedFindOrCreate
  def find_and_update_or_create(key_attributes, items_attributes, options = {})
    ...
    fetch_items_attributes(items_attributes, key_attributes, match_with_all_attributes, options)
  end
  alias :update_or_create :find_and_update_or_create
  alias :create_or_update :find_and_update_or_create

  def fetch_items_attributes(items_attributes, key_attributes, match_attributes, options)
    items_attributes.each_with_index.collect do |item_attributes, item_index|
      key_values = match_attributes ? item_attributes : item_attributes.slice(*key_attributes)
      ...
      create_or_update_record(item, item_attributes, options)
    end
  end

  def create_or_update_record(item, item_attributes, options)
